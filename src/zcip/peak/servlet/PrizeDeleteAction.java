package zcip.peak.servlet;

import java.io.IOException;

import javax.servlet.ServletException;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import zcip.peak.service.ICupService;
import zcip.peak.service.IPrizeService;
import zcip.peak.service.impl.CupService;
import zcip.peak.service.impl.PrizeService;

public class PrizeDeleteAction extends HttpServlet {
	
	private IPrizeService iprizeservice = new PrizeService();
	
	private ICupService icupservice = new CupService();

	public void doGet(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
				
		String pid=request.getParameter("pid");
		if(icupservice.selectPid(pid)==true){
			request.getSession().setAttribute("deletePrizeError", "deletePrizeError");
		}
		else {
			iprizeservice.deletePrize(pid);
			request.getSession().setAttribute("deletePrizeError", null);
		}
		
		
		response.sendRedirect(request.getContextPath() + "/prize");
		
	}

	public void doPost(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		request.setCharacterEncoding("utf-8");
		doGet(request,response);
	}

}
